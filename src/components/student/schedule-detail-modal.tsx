...
import { BookText, Calendar, Clock, GraduationCap, Link2, UserRound } from "lucide-react";

const statusStyles: Record<string, string> = {
  scheduled: "bg-success/10 text-success border-success/20",
  cancelled: "bg-destructive/10 text-destructive border-destructive/20",
  completed: "bg-low/10 text-low border-low/20",
};
...
export function StudentScheduleDetailModal({
  open,
  onClose,
  schedule,
}: {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Tutor */}
          <div className="flex flex-col gap-1.5">
            <p className="text-xs font-semibold text-low uppercase tracking-wider">Tutor</p>
            ...
            <div className="flex items-center gap-3 bg-offwhite rounded-lg p-3.5 border border-shade-2">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium text-high truncate">
                  {schedule.user?.firstName ?? "Tutor"} {schedule.user?.lastName ?? ""}
                </p>
                <p className="text-xs text-low truncate">{schedule.user?.email ?? ""}</p>
              </div>
            </div>
          </div>
          ...
          {/* Student */}
          <div className="flex flex-col gap-1.5">
            <p className="text-xs font-semibold text-low uppercase tracking-wider">Student</p>
            ...
            <div className="flex items-center gap-3 bg-offwhite rounded-lg p-3.5 border border-shade-2">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium text-high truncate">
                  {schedule.student?.firstName ?? "Student"} {schedule.student?.lastName ?? ""}
                </p>
                <p className="text-xs text-low truncate">{schedule.student?.email ?? ""}</p>
              </div>
            </div>
          </div>
          ...
          {schedule.course?.description && (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-start gap-3 bg-offwhite rounded-lg p-3.5 border border-shade-2">
                <div className="w-9 h-9 rounded-full bg-orange/10 flex items-center justify-center shrink-0 mt-0.5">
                  <BookText className="w-4.5 h-4.5 text-orange" />
                </div>

                <p className="text-sm text-low leading-relaxed">{schedule.course.description}</p>
              </div>
            </div>
          )}
        </div>
