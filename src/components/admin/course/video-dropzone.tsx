...
export function VideoDropzone({ onFileDrop, onClear, value }: DropzoneProps) {
  ...
  useEffect(() => {
    setPreviews(value);
    setVideoDuration(value?.preview ? Number(value.preview) : 0);
  }, [value]);
          ...
          <div className="relative p-4 h-40 w-72 border border-shade-2 rounded-xl">
            <div className="flex flex-col gap-2">
              <p className="font-medium text-sm truncate">
                {previews.file ? previews.file.name : "Existing video"}
              </p>
